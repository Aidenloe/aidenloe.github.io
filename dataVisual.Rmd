...
The `geom_vline()` can be used to represent the vertical line. Since we are manipulating a specific variable in the dataset, we need to put it inside the `aes()`. 
...
You will need the help of google to help you for some of the exericses. The idea is to basically recreate the plots you see below. 

To ease the workload, I have written the structure of the code for you. In the script, you will see many XXXXs  and your job is to remove the XXXXs and fill it in with the correct codes in order for the graph to be displayed.
...
```{r dataset ggplot questions, result='hide'}
require(reshape)
#### Load data set ####
Big5mfq <- big5

########################################### Big 5 & mfq (ggplot2 set)   #####################################################
