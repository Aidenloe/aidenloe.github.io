...
```{r median time, echo=F, fig.align='center', fig.width=3, fig.height=4}
dat %>%
  group_by(University, research_period) %>%
  count() %>%
  as.data.frame()-> count_date
...
```
```{r research period, echo=F}
dat %>%
  select(University, research_period)%>% ## need to have select()!!! 
  group_by(University, research_period) %>%
  count() %>%
  as.data.frame()-> count_date
...
4. Having a lower project cost not give you automatically give you a higher score over others. 
