...
Now, bear in mind that a D constant is added to these formulaes to allow for the logistic distribution to approximate to a normal distribution. If you wish to based your calculations on a logistic Distribution instead, remove the constant $D$ in the formulas. 
...
You can see that it is exactly the same as a 2PL model when the value of the discrimination ($a$ parameter) is set to 1.
...
<br/>
<center>$\large{I_{i\theta} = \frac{D^2a_i^2D_{i\theta}[P_{i\theta}-c_i]^2}{P_{i\theta(1-c_i)^2}}}$</center><br/>

You can observe that the formula is a little bit more complex, but it really only introduces one new **$c$ parameter** in the equation. The $c$ parameter is also the **guessing parameter** which is typically fixed to 0 in both the 1PL and the 2PL model. <br/>


<br/>
<center>Let's attempt an example to calculate item and test information based on the <span style="color:red"> Rasch model</span> in R. </center> <br/>


```{r single item information function}

# Given that the difficulty parameter for an item is -1.5
b <- -1.5

# Create item information function for a Rasch model 
i_info <- function(b,a=1,θ=seq(-4,4,length.out=1000)){
P <- NULL 
Q <- NULL
D <- 1.702
Ii <- NULL
for(i in 1:1000){
  ...
  Q[i]= 1-P[i]
  Ii[i] =D^2*P[i]* Q[i] # (1PL)
   }
return(Ii)
}

# Get a single item information  based on a rasch model
itemInfo<- i_info(b = b, a = 1, θ=seq(-4,4,length.out=1000))
...
To translate the amount of information into a standard error of estimation, you only need to take the reciprocal of the square root of the amount of test information. You will notice that the SE values are extremely high. This is expected since we are only using two items to calculate test information. As the number of items increases, more test information is provided, the resulting SE will becoming smaller and smaller. 
